defmodule CortexCore.Workers.Adapters.AnthropicWorkerTest do
  ...
  describe "health_check/1" do
    ...
    test "returns error when API is not accessible", %{worker: worker, bypass: bypass} do
      Bypass.expect_once(bypass, "POST", "/v1/messages", fn conn ->
        Plug.Conn.resp(conn, 500, Jason.encode!(%{
          "error" => %{
  ...
  describe "stream_completion/3" do
    test "handles successful streaming response", %{worker: worker, bypass: bypass} do
      ...
      Bypass.expect_once(bypass, "POST", "/v1/messages", fn conn ->
        ...
        {:ok, conn} = Plug.Conn.chunk(conn, "event: message_start\ndata: #{Jason.encode!(%{
          "type" => "message_start",
          "message" => %{"id" => "msg_test"}
        })}\n\n")
        
        {:ok, conn} = Plug.Conn.chunk(conn, "event: content_block_delta\ndata: #{Jason.encode!(%{
          "type" => "content_block_delta",
          "delta" => %{"text" => "Hello"}
        })}\n\n")
        
        {:ok, _conn} = Plug.Conn.chunk(conn, "event: message_stop\ndata: #{Jason.encode!(%{
          "type" => "message_stop"
        })}\n\n")
      end)
  ...
  describe "priority/1" do
    test "returns appropriate priority for different models", %{} do
      claude_4_worker = AnthropicWorker.new([
        name: "claude_4",
        api_keys: ["key"],
        default_model: "claude-sonnet-4-20250514"
      ])

      claude_3_worker = AnthropicWorker.new([
        name: "claude_3",
        api_keys: ["key"],
      ...
      assert AnthropicWorker.priority(claude_4_worker) == AnthropicWorker.priority(claude_3_worker)
    ...
    test "extracts text from content blocks", %{worker: worker, bypass: bypass} do
      ...
      Bypass.expect_once(bypass, "POST", "/v1/messages", fn conn ->
        ...
        Enum.reduce(events, conn, fn event, acc_conn ->
          {:ok, new_conn} = Plug.Conn.chunk(acc_conn, "event: #{event["type"]}\ndata: #{Jason.encode!(event)}\n\n")
          new_conn
        end)
      end)
